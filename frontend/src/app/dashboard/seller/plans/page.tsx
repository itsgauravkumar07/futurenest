...
// Shown after a seller picks a plan, before any purchase exists in the
...
export default function SellerPlansPage() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([api.get("/plans?audience=seller"), api.get("/plans/my-purchases")])
  ...
  // Item 4: once a seller has ANY purchased plan (pending or active), block
  // buying another until it resolves (activated+expired/refunded, or cancelled).
  const disablePurchasing = hasPendingPurchase || user?.planStatus === "active";

  return (
    <div>
      <h1 className="text-2xl">My Plan</h1>
          ...
          {plans.map((plan) => (
            <div key={plan._id} className="card flex flex-col p-5">
              ...
              <p className="mt-1 text-xs uppercase tracking-wide text-slate">
                {plan.targetListingType === "both" ? "Sale + Rental" : plan.targetListingType}
              </p>
              ...
              <ul className="mt-3 flex-1 space-y-1 text-sm text-slate">
                ...
                <li>{plan.qualifiedLeadsLimit} qualified leads</li>
